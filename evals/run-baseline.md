...
### 无头 CLI 不可用时的替代路径（2026-09-10 实测）

`AGENT_CMD` 依赖可无头调用的被测 Agent。本机实测 `claude` CLI 未登录且无 `ANTHROPIC_API_KEY` / `OPENAI_API_KEY`，`AGENT_CMD` 路径跑不通。替代做法：

- **被测**：改用**独立上下文子 Agent**（每个任务一个，互不共享上下文），工作区 `evals/.runs/<commit>/<task>/run1/ws`；prompt 只给三样——加载指令（`AGENT.md` → `skills/rd-digital-agent/SKILL.md` → 分派）、**固定输入原文**、「严禁读 `evals/`」。judge（报告作者）不进入被测会话，**judge 隔离仍然成立**。
- **无人值守**：子 Agent 单轮执行，无法真的等待人回答。需在 prompt 中约定：人审节点以「待你确认 + 显式推荐假设」写入产物，假设被推翻即回拉。

**已知偏差（必须在报告里标注，否则趋势线失真）**：

| 偏差 | 影响 |
|---|---|
| 模型/温度由宿主调度，未固定 | 随机性未抑制，跨版本不可比 |
| 无人值守，人审无法真正停顿 | **D3 系统性偏低** |
| 单会话，无法 spawn 子 Agent | **D5 系统性偏低** |
| 每任务 1 次 | 未取均值 |

> 结论：该路径可用于「跑通流程 + 发现结构性问题」，但 D3/D5 不可作为回归基线。无头 CLI 恢复后优先切回 `AGENT_CMD`。用例见 `evals/reports/6373b99-2026-09-10.md`。
