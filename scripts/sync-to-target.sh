...
sync_one() {
  ...
  (
    ...
    # sync/agent-kit 由本脚本独占：每轮都从基线重建，与远端既有同名分支是「兄弟提交」而非
    # 「祖先-后代」，普通 push 必被拒（非快进），故必须 --force。
    # 不能用 --force-with-lease：本脚本是按 URL（而非 remote 名）推送的，git 找不到可校验的
    # remote-tracking ref，会直接以「(stale info)」拒绝，等于永远推不上去。
    git push --quiet --force "https://x-access-token:${SYNC_TOKEN}@github.com/${repo}.git" "$PR_BRANCH" \
      || { echo "::error::推送失败（SYNC_TOKEN 无写权限，或分支受保护）：${repo}" >&2; exit 1; }
